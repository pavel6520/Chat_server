...
function authFunc(){
	var json = struct.CWLoginWithPass(document.getElementById("authLogin").value, document.getElementById("authPass").value);
...
function regFunc() {
	var json = struct.CWReg(document.getElementById("regLogin").value, document.getElementById("regPass").value, document.getElementById("regEmail").value);
...
function enterChat(){
	...
	ws.onmessage = function(evt) { 
		...
		if (json.mestype == 'mesToClient') {
			...
			$('#ChatBodyList2').append('<li class="ChatBodyMessage"><div class="ChatBodyMessageLogin">' + json.from + 
